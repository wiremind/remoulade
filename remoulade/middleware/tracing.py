...
class OpenTelemetryMiddleware(Middleware):
    ...
    def __init__(
        ...
        *,
        ...
        ----------
        tracer:
            An ``opentelemetry.trace.Tracer`` instance.  When provided,
            *tracer_name* and *schema_url* are ignored.
        tracer_name:
    ...
    def _start_span(
        ...
        *,
        ...
        """Create a span, activate it and store it in the registry."""
        retry_count = message.options.get("retries", 0)

        span = self._tracer.start_span(
            self._operation_name(action, retry_count),
            context=context,
            kind=kind,
            links=links,
            attributes={
                "remoulade.retry_count": retry_count,
                "remoulade.action": action,
        ...
        self._span_registry[message.message_id, is_publish] = (span, activation)
    ...
    def after_process_message(self, broker, message, *, result=None, exception=None):
        self._end_span(message.message_id, is_publish=False, exception=exception)

    def after_skip_message(self, broker, message):
        self._end_span(message.message_id, is_publish=False)

    def after_message_canceled(self, broker, message):
        self._end_span(message.message_id, is_publish=False)

    def _end_span(self, message_id: str, *, is_publish: bool, exception: BaseException | None = None):
        span_and_activation = self._span_registry.pop((message_id, is_publish), None)
    ...
    def after_enqueue(self, broker, message, delay, exception=None):
        self._end_span(message.message_id, is_publish=True, exception=exception)
