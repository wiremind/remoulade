...
class TestMessageState:
    ...
    def test_pending_state_message(
        self, stub_broker, state_middleware, do_work
    ):
        msg = do_work.send()
        assert (
            state_middleware.backend.get_state(msg.message_id).name == StateNamesEnum.Pending
        )

    def test_success_state_message(
        self, stub_broker, stub_worker, state_middleware, do_work
    ):
        msg = do_work.send()
        stub_broker.join(do_work.queue_name)
        stub_worker.join()
        assert (
            state_middleware.backend.get_state(msg.message_id).name == StateNamesEnum.Success
        )

    def test_started_state_message(
        self, stub_broker, state_middleware, stub_worker
    ):
        @remoulade.actor
        def wait():
            time.sleep(0.3)
        ...
        time.sleep(0.1)
        assert (
            state_middleware.backend.get_state(msg.message_id).name == StateNamesEnum.Started
        )

    def test_failure_state_message(
        self, stub_broker, stub_worker, state_middleware
    ):
        @remoulade.actor
        def error():
            raise Exception()
        ...
        stub_worker.join()
        assert (
            state_middleware.backend.get_state(msg.message_id).name == StateNamesEnum.Failure
        )
    ...
    def test_cancel_state_message(
        ...
        stub_worker.join()
        assert (
            state_middleware.backend.get_state(msg.message_id).name == StateNamesEnum.Canceled
        )

    def test_skip_state_message(
        self, stub_broker, stub_worker, state_middleware, do_work
    ):
        ...
        stub_worker.join()
        assert (
            state_middleware.backend.get_state(msg.message_id).name == StateNamesEnum.Skipped
        )

    @pytest.mark.parametrize(
        "ttl, result_type",
        [
            pytest.param(1000, State),
            pytest.param(1, type(None)),
        ],
    )
    def test_expiration_data_backend(
        self, ttl, result_type, stub_broker, state_backend
    ):
        @remoulade.actor
        def wait():
            time.sleep(0.3)
    ...
    def test_maximum_size(
        self, max_size, expected, stub_broker, state_backend, do_work
    ):
        @remoulade.actor
        def do_work(x):
            return x
